...
class ChatSimple {
  ...
  public function __construct($chatrooms) {
    ...
    if(defined('CHATDIR')) $this->chatdir = _ROOT.CHATDIR;
	...
	if( isset($_SESSION['_login']['user_id']) ){
		$this->chatadd	= 1;
	} 
	
	if( !file_exists($this->fileroom) ){
		file_put_contents($this->fileroom);
		chmod($this->fileroom,777);	
	}
  ...
  protected function setChatRoomCnt() {
    ...
    if(file_put_contents($this->fileroom, $rechat)) {
		return $rechat;
	}else{
		return json_encode(array('error'=>sprintf($this->lsite['err_savechat'], $this->fileroom)));
	}
  }
